class Events::CountriesController < ApplicationController
  ...
  def index
    ...
    @events_by_country = Event.all.sort_by { |event| event.static_metadata&.home_sort_date }.reverse.group_by { |event| event.static_metadata&.country || "Unknown" }.sort_by { |key, value| key&.iso_short_name || "ZZ" }.to_h
  end
